...
library(pacman)
p_load(knitr,DT,ggridges,lmSupport,ggplot2, dplyr,tidyr,MASS,sjmisc,tidyverse,plyr,summarytools)
...
plot <-ggplot(dt1long,aes(x=task1PercentDemocratC,y=task1Version,fill=task1Version))+
    ggridges::stat_density_ridges()+
    scale_fill_manual(values=rev(c("light green","green", "light blue", "blue", "grey")))+
...
plotF <-ggplot(data=dfPlot,aes(x=task1Version,y=Fvalue4,fill=task1Version,color=task1Version))+
  ...
  ylab("Perceived Polarization\n(Participants' F-values)")+scale_x_discrete(labels=c("Uniform Grey","Continuous\nRed-Blue","Dichotomous\nRed-Blue","Continuous\nOrange-Green","Dichotomous\nOrange-Green"))+
  scale_fill_manual(values=c("grey","#A8AFEA","#3267D2","#C9DFC5","#077F21"))+
  scale_color_manual(values=c("grey","#A8AFEA","#3267D2","#C9DFC5", "#077F21"))+
    scale_y_continuous(breaks=c(0,.25^.25,1^.25,  4^.25,10^.25,40^.25),labels=c(0,.25,1,4,10,40))
plotF
ggsave("task1_results.png",plotF)
```
