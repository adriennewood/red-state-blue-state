---
title: "Red state, blue state: Do dichotomized political maps increase perceived polarization? *Planned analyses for Stage 1 Registered Report*"
author: "Rémy A. Furrer, Adrienne Wood, Karen Schloss, Gary Lupyan, & Paula M. Niedenthal"
date: "5/11/2020"
output: 
  html_document:
      toc: true
...
df$colorGradientN <-varRecode(df$colorGradient,c("dichotomous","continuous","none"),c(-.5,.5,0))
...
df2$colorCond<-as.factor(ifelse(grepl("red", df2$experimentalCond, fixed=TRUE)|grepl("blue", df2$experimentalCond, fixed=TRUE),"redBlue",
                        ifelse(grepl("yellow", df2$experimentalCond, fixed=TRUE)|grepl("green", df2$experimentalCond, fixed=TRUE),"yellowGreen","none"))) 

df2$majorityCond <-ifelse(grepl("Dem",df2$experimentalCond,fixed=TRUE),-.5,
                          ifelse(grepl("Rep",df2$experimentalCond,fixed=TRUE),.5,NA))
```

In our central analysis for Task 2, we'll use redBlue as the reference level for our colorCond factor. That way the main effect of majorityCond will be the key test of whether the red or blue coloring biases people's estimates. Again, we will use modelAssumptions to visualize the residuals of the model and adjust the model or the variables as necessary. We will conduct parallel analyses with a) the participants' estimates of how likely the target is to vote, period, and b) how important it is that the target vote as the outcome variables. Those variables were not simulated here.
```{r task 2 regression}

df2$colorCond<-relevel(df2$colorCond,ref="redBlue")
